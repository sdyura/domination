...
public class NNTrainer extends TrainerBase {
	...
	protected void trainFromFile(String filename) {
		...
		try {
			...
			System.out.println("NNTraining started");
			// Start training
			
			FeedForwardNetwork nnet = new FeedForwardNetwork(inputNodes,hiddenNodes,outputNodes,0.03f);
			for (int i = 0; i < 50; i++) {
				System.out.print("NNTraining iteration: "+i);
				float error = 0;
				// The training set:
				for (int j = 0; j < inputData.size(); j++) {
					error += nnet.trainNetwork((float[])inputData.get(j),(float[])targetData.get(j), ((Boolean)winnerData.get(j)).booleanValue());
				}
